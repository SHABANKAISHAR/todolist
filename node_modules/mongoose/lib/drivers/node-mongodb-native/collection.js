...
const { MongooseCollection } = require('mongoose'); // Ensure you import MongooseCollection correctly

function NativeCollection(name, conn, options) {
  this.collection = null; // Initialize to null, but it should be properly set up later.
  
  this.Promise = options.Promise || Promise; // Use provided Promise or default to native Promise
  
  const modelName = options.modelName; // Extract modelName
  this.modelName = modelName;
  
  const newOptions = { ...options }; // Create a shallow copy of options
  delete newOptions.modelName; // Remove modelName from the shallow copy

  this._closed = false; // Indicates if the collection is closed

  MongooseCollection.apply(this, [name, conn, newOptions]); // Apply MongooseCollection properly
}
